...
class Timer(tk.Frame):  # This class creates and manages all the widgets such as button, entry boxes, and the progressbar
    ...
    def createWidgets(self):
        ...
        self.hour.place(x=65,
                        y=300)  ## Used .place instead of grid because grid would allow boxes only at the very bottom of page.
        ...
        self.progressbar = CircularProgressbar(self.canvas, 150, 0, 350, 200,
                                               20)  # this will create an empty progress bar that will not start until start is pressed.
    ...
    def start(self):  # Start button pressed, so start the countdown
        ...
        if self.progressbar.running == False:  # This checks that the timer is not running which is should not be until time is inputted and start is pressed
            totaltime = int(self.hour.get()) * 3600 + int(self.minute.get()) * 60 + int(
                self.sec.get())  # Get the total time which is going to be sent to the progressbar.start function
